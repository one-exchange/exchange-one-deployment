...
// The OneX token
contract OneX is ERC20("ExchangeOne", "ONEx"), Ownable, Authorizable {
    uint256 private _cap = 21_000_000e18;
    uint256 private _totalLock;
    uint256 public lockFromBlock;
    uint256 public lockToBlock;
    uint256 public manualMintLimit = 2100e18;
    ...
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (RewardsGenerator).
    ...
    function delegateBySig(
        ...
        require(signatory != address(0), "OneX::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "OneX::delegateBySig: invalid nonce");
        require(now <= expiry, "OneX::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, "OneX::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(block.number, "OneX::_writeCheckpoint: block number exceeds 32 bits");
