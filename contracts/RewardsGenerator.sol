...
// RewardsGenerator generates OneX rewards.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Exchange One is sufficiently
// distributed and the community can show to govern itself.
//
contract RewardsGenerator is Ownable, Authorizable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of OneXs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accOneXTokenPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accOneXTokenPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. OneXs to distribute per block.
        uint256 lastRewardBlock; // Last block number that OneXs distribution occurs.
        uint256 accOneXTokenPerShare; // Accumulated OneXs per share, times 1e12. See below.
    }

    // The Governance token
    OneX public oneXToken;
    ...
    // OneXs created per block.
    uint256 public REWARD_PER_BLOCK;
    // Bonus muliplier for early OneX makers.
    ...
    // The block number when OneX mining starts.
    ...
    event SendOneXReward(address indexed user, uint256 indexed pid, uint256 amount, uint256 lockAmount);

    modifier nonDuplicated(IERC20 _lpToken) {
        require(poolExistence[_lpToken] == false, "RewardsGenerator::nonDuplicated: duplicated");
        _;
    }
    ...
    function add(
        ...
        require(poolId1[address(_lpToken)] == 0, "RewardsGenerator::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
        ...
        poolInfo.push(
            PoolInfo({
                ...
                accOneXTokenPerShare: 0
            })
        );
    }

    // Update the given pool's OneX allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 OneXTokenForDev;
        uint256 OneXTokenForFarmer;
        uint256 OneXTokenForLP;
        uint256 OneXTokenForCom;
        uint256 OneXTokenForFounders;
        (OneXTokenForDev, OneXTokenForFarmer, OneXTokenForLP, OneXTokenForCom, OneXTokenForFounders) = getPoolReward(
            pool.lastRewardBlock,
            block.number,
            pool.allocPoint
        );
        oneXToken.mint(address(this), OneXTokenForFarmer);
        pool.accOneXTokenPerShare = pool.accOneXTokenPerShare.add(OneXTokenForFarmer.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
        if (OneXTokenForDev > 0) {
            oneXToken.mint(address(devaddr), OneXTokenForDev);
            ...
            if (block.number <= FINISH_BONUS_AT_BLOCK) {
                oneXToken.lock(address(devaddr), OneXTokenForDev.mul(75).div(100));
            }
        }
        if (OneXTokenForLP > 0) {
            oneXToken.mint(liquidityaddr, OneXTokenForLP);
            ...
            if (block.number <= FINISH_BONUS_AT_BLOCK) {
                oneXToken.lock(address(liquidityaddr), OneXTokenForLP.mul(45).div(100));
            }
        }
        if (OneXTokenForCom > 0) {
            oneXToken.mint(comfundaddr, OneXTokenForCom);
            //Community Fund has xx% locked during bonus period and then drips out linearly over 3 years.
            if (block.number <= FINISH_BONUS_AT_BLOCK) {
                oneXToken.lock(address(comfundaddr), OneXTokenForCom.mul(85).div(100));
            }
        }
        if (OneXTokenForFounders > 0) {
            oneXToken.mint(founderaddr, OneXTokenForFounders);
            ...
            if (block.number <= FINISH_BONUS_AT_BLOCK) {
                oneXToken.lock(address(founderaddr), OneXTokenForFounders.mul(95).div(100));
            }
        }
    }
        ...
        )
    {
        ...
        uint256 OneXCanMint = oneXToken.cap().sub(oneXToken.totalSupply());

        if (OneXCanMint < amount) {
            forDev = 0;
            forFarmer = OneXCanMint;
            forLP = 0;
            forCom = 0;
            forFounders = 0;
        } else {
    ...
    // View function to see pending OneXs on frontend.
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accOneXTokenPerShare = pool.accOneXTokenPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            uint256 OneXTokenForFarmer;
            (, OneXTokenForFarmer, , , ) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
            accOneXTokenPerShare = accOneXTokenPerShare.add(OneXTokenForFarmer.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accOneXTokenPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accOneXTokenPerShare).div(1e12).sub(user.rewardDebt);
            uint256 masterBal = oneXToken.balanceOf(address(this));
            ...
            user.rewardDebt = user.amount.mul(pool.accOneXTokenPerShare).div(1e12);
        }
    }
    ...
    // Deposit LP tokens to RewardsGenerator for OneX allocation.
    function deposit(
        uint256 _pid,
        uint256 _amount,
        address _ref
    ) public nonReentrant {
        require(_amount > 0, "RewardsGenerator::deposit: amount must be greater than 0");
        ...
        devr.rewardDebt = devr.amount.mul(pool.accOneXTokenPerShare).div(1e12);
    ...
    // Withdraw LP tokens from RewardsGenerator.
    function withdraw(
        uint256 _pid,
        uint256 _amount,
        address _ref
        ...
        UserGlobalInfo storage refer = userGlobalInfo[_ref];
        UserGlobalInfo storage current = userGlobalInfo[msg.sender];
        require(user.amount >= _amount, "RewardsGenerator::withdraw: not good");
        if (_ref != address(0)) {
            refer.referrals[msg.sender] = refer.referrals[msg.sender] - _amount;
            refer.globalRefAmount = refer.globalRefAmount - _amount;
        }
    ...
    // Safe OneXToken transfer function, just in case if rounding error causes pool to not have enough OneXTokens.
    function safeOneXTokenTransfer(address _to, uint256 _amount) internal {
        uint256 oneXTokenBal = oneXToken.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > oneXTokenBal) {
            transferSuccess = oneXToken.transfer(_to, oneXTokenBal);
        } else {
            transferSuccess = oneXToken.transfer(_to, _amount);
        }
        require(transferSuccess, "RewardsGenerator::safeOneXTokenTransfer: transfer failed");
    }
    ...
    function reclaimTokenOwnership(address _newOwner) public onlyAuthorized() {
        oneXToken.transferOwnership(_newOwner);
    }
}
